...
#        'currency_id': fields.related('journal_id','currency', type='many2one', relation='res.currency', string='Currency', store=True, readonly=True, states={'draft':[('readonly',False)]}),
    ...
    def onchange_journal_voucher(self, cr, uid, ids, line_ids=False, tax_id=False, price=0.0, partner_id=False, journal_id=False, ttype=False, context=None):
        ...
        #netsvc.Logger().notifyChannel("onchange_journal_voucher", netsvc.LOG_INFO, "ids=%s, line_ids=%s, tax_id=%s, price=%s, partner_id=%s, journal_id=%s, ttype=%s, context=%s" %(ids, line_ids, tax_id, price, partner_id, journal_id, ttype, context))
    ...
    def onchange_journal(self, cr, uid, ids, journal_id, line_ids=False, tax_id=False, partner_id=False, context=None):
        if not journal_id:
            return False
        journal_pool = self.pool.get('account.journal')
        journal = journal_pool.browse(cr, uid, journal_id, context=context)
        ...
        if account_id :
            ...
            if account_id.tax_ids:
                #YT 2/12/2011
                # Fix it, only support one payment tax
                #tax_id = account_id.tax_ids[0].id
                for tax in account_id.tax_ids:
                    if tax.payment:
                        tax_id = tax.id
    ...
    def action_move_line_create(self, cr, uid, ids, context=None):
        ...
        for inv in self.browse(cr, uid, ids, context=context):
            ...
            current_currency = inv.currency_id and inv.currency_id.id or company_currency
            inv_currency_id = current_currency
            debit = 0.0
            credit = 0.0
            ...
            #netsvc.Logger().notifyChannel("action_move_line_create", netsvc.LOG_INFO, "current_currency:%s, company_currency:%s, move_line: %s" % (current_currency, company_currency,move_line))
            move_line1_id = move_line_pool.create(cr, uid, move_line)
            rec_list_ids = []
            line_total = debit - credit
            if inv.type == 'sale':
                line_total = line_total - currency_pool.compute(cr, uid, inv_currency_id, company_currency, inv.tax_amount, context=context_multi_currency)
            elif inv.type == 'purchase':
                line_total = line_total + currency_pool.compute(cr, uid, inv_currency_id, company_currency, inv.tax_amount, context=context_multi_currency)
            #YT 8/12/2011 transfers module
            currency_amount = 0.0
            company_amount = 0.0
            ...
            for line in inv.line_ids:
                ...
                if not line.amount:
                    continue
                #YT 8/12/2011 transfers module
                if self.pool.get('account.voucher').fields_get(cr,uid,['transfer'],context=context) and inv.transfer:
                    current_currency = line.currency_id and line.currency_id.id or current_currency
                    currency_amount += line.currency_amount
                    if inv.currency_id.id == company_currency:
                        line_company_amount = line.currency_amount
                    elif line.currency_id.id == company_currency:
                        line_company_amount = line.amount
                    else:
                        line_company_amount = currency_pool.compute(cr, uid, inv_currency_id, company_currency, line.currency_amount, context=context_multi_currency)
                    company_amount += line_company_amount
            ...
            #YT 8/12/2011 transfers module
            if self.pool.get('account.voucher').fields_get(cr,uid,['transfer'],context=context) and inv.transfer:
                if inv_currency_id <> company_currency:
                    move_line_pool.write(cr, uid, [move_line1_id],{'amount_currency':(currency_amount * -1), 'credit':company_amount,})
            
            #if not currency_pool.is_zero(cr, uid, inv_currency_id, line_total):
            elif not currency_pool.is_zero(cr, uid, inv.currency_id, line_total):
                diff = line_total
                account_id = False
            ...
            self.write(cr, uid, [inv.id],{'move_id': move_id,'state': 'posted','number': name,})
