...
def _check_membership(self, cr, uid, data, context):
    ...
    partner_ids = pooler.get_pool(cr.dbname).get('res.partner').search(cr, uid, ['|',('active','=','True'),('active','=','False')], context=context)
    ...
    if partner_ids:
        ...
        final_fields['final_text']['value'] = str(final_text.encode('ascii','ignore'))
        final_fields['final_count']['value'] = len(changed_lines)
...
class wizard_check_membership(wizard.interface):
    ...
    states = {
        'init' : {
            ...
            'result' : {
                ...
                'arch' : wizard_arch,
                'fields' : {},
                'state' : [('end', 'Cancel'),('do_it', 'Confirm') ]}
        },
        'do_it' : {
            'actions' : [_check_membership],
            'result' : { 'type': 'state', 'state':'show' }
        },
        'show' : {
            'actions': [],
            'result' : {'type' : 'form',
                        'arch': wizard_final_arch,
                        'fields' : final_fields,
                        'state' : [('end','Close')],
            },
        },
...
wizard_check_membership("cci_wizard_check_membership")
