...
class partner_commercial(report_sxw.rml_parse):
    ...
    def _get_participations(self,partner_id):
        result = []
        self.cr.execute("SELECT cl.name as name, p.date_in, p.date_out, c.name || ' ' || c.first_name as who, " \
                               "pstate.name as state_name, pstate.current as state_current " \
        ...
        for line in lines:
            ...
            if line['date_in'] and line['date_out']:
                values['dates'] = line['date_in'][8:10]+'/'+line['date_in'][5:7]+'/'+line['date_in'][0:4] + ' -> ' + line['date_out'][8:10]+'/'+line['date_out'][5:7]+'/'+line['date_out'][0:4]
            elif line['date_in']:
    ...
    def _get_interest(self,partner_id):
        return [{'date':'31-03-2011','contact':'Simon Micha','product':'Table et cotisation','turnover':1200.0,'desc':'test de description'},]

    def _get_histo(self,partner_id):
        return [{'date':'31-03-2011','contact':'Simon Micha','product':'Table et cotisation','desc':'test de description'},]

report_sxw.report_sxw('report.partner.commercial', 'res.partner',
    'addons/cci_last_module/report/partner_commercial.rml',parser=partner_commercial,
    header=False)
